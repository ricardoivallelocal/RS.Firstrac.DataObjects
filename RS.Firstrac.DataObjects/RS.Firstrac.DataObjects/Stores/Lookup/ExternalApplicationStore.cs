...
namespace RS.Firstrac.DataObjects.Stores.Lookup
{
    ...
    public class ExternalApplicationStore : StoreBase, IExternalApplicationStore
    {
        ...
        public async Task<IAPIOperationResult<IEnumerable<IExternalApplication>>> GetAll(bool? activeOnly, Dictionary<string, object>? filterBy = null, bool? exactMatch = true, bool? mutuallyExclusive = false, bool? includeNavigationProperties = true)
        {
            if (filterBy?.Any() ?? false)
                return await _firstracApiHelper.PostAsync<Dictionary<string, object>, APIOperationResult<IEnumerable<IExternalApplication>>>($"api/ExternalApplication/filteredBy?activeOnly={activeOnly}&exactMatch={exactMatch}&mutuallyExclusive={mutuallyExclusive}&includeAllNavigationProperties={includeNavigationProperties}", filterBy);
            else
        ...
        {
            return await _firstracApiHelper.PostAsync<Dictionary<string, object>, APIOperationResult<IEnumerable<IDropdownItem>>>($"api/ExternalApplication/dropdownItems?exactMatch={exactMatch}", filterBy);
        }
