...
namespace RS.Firstrac.DataObjects.Stores.Lookup
{
    ...
    public class HandlingDestinationStore : StoreBase, IHandlingDestinationStore
    {
        ...
        public async Task<IAPIOperationResult<IEnumerable<IHandlingDestination>>> GetAll(bool? activeOnly, Dictionary<string, object>? filterBy = null, bool? exactMatch = true, bool? mutuallyExclusive = false, bool? includeNavigationProperties = true, Dictionary<string,object>? dependencies = null)
        {
            if (filterBy?.Any() ?? false)
            {
                ...
                return await _firstracApiHelper.PostAsync<IGetAllRequest, APIOperationResult<IEnumerable<IHandlingDestination>>>($"api/HandlingDestination/filteredBy", request);              
            }
        ...
        public override async Task<IAPIOperationResult<IEnumerable<IDropdownItem>>> GetForDropdown(Dictionary<string, object>? filterBy, bool exactMatch = false, Dictionary<string,object>? dependencies = null)
        {
            ...
            return await _firstracApiHelper.PostAsync<IGetForDropdownRequest, APIOperationResult<IEnumerable<IDropdownItem>>>($"api/HandlingDestination/dropdownItems", request);
        }
