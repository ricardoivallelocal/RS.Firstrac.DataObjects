...
namespace RS.Firstrac.DataObjects.Stores.Lookup
{
    public class ProductServiceTypeStore : StoreBase, IProductServiceTypeStore
    {
        ...
        public async Task<IAPIOperationResult<IEnumerable<IProductServiceType>>> GetAll(bool? activeOnly, Dictionary<string, object>? filterBy = null, bool? exactMatch = true, bool? mutuallyExclusive = false, bool? includeNavigationProperties = true, Dictionary<string,object>? dependencies = null)
        {
            if (filterBy?.Any() ?? false)
            {
                ...
                return await _firstracApiHelper.PostAsync<IGetAllRequest, APIOperationResult<IEnumerable<IProductServiceType>>>($"api/ProductServiceType/filteredBy", request);
        ...
        public async Task<IAPIOperationResult<IEnumerable<IDropdownItem>>> GetForDropdown(Dictionary<string, object>? filterBy, bool exactMatch = false, Dictionary<string,object>? dependencies = null)
        {
            ...
            return await _firstracApiHelper.PostAsync<IGetForDropdownRequest, APIOperationResult<IEnumerable<IDropdownItem>>>($"api/ProductServiceType/dropdownItems", request);
        }
