...
namespace RS.Firstrac.DataObjects.Stores.Lookup
{
    ...
    public class InvoiceDeliveryOptionStore : StoreBase, IInvoiceDeliveryOptionStore
    {
        ...
        public async Task<IAPIOperationResult<IEnumerable<IInvoiceDeliveryOption>>> GetAll(bool? activeOnly, Dictionary<string, object>? filterBy = null, bool? exactMatch = true, bool? mutuallyExclusive = false, bool? includeNavigationProperties = true)
        {
            if (filterBy?.Any() ?? false)
                return await _firstracApiHelper.PostAsync<Dictionary<string, object>, APIOperationResult<IEnumerable<IInvoiceDeliveryOption>>>($"api/InvoiceDeliveryOption/filteredBy?activeOnly={activeOnly}&exactMatch={exactMatch}&mutuallyExclusive={mutuallyExclusive}&includeAllNavigationProperties={includeNavigationProperties}", filterBy);
            else
        ...
        {
            return await _firstracApiHelper.PostAsync<Dictionary<string, object>, APIOperationResult<IEnumerable<IDropdownItem>>>($"api/InvoiceDeliveryOption/dropdownItems?exactMatch={exactMatch}", filterBy);
        }
