...
namespace RS.Firstrac.DataObjects.Stores.Lookup
{
    public class AccountTypeStore : StoreBase, IAccountTypeStore
    {
        ...
        public async Task<IAPIOperationResult<IEnumerable<IAccountType>>> GetAll(bool? activeOnly, Dictionary<string, object>? filterBy = null, bool? exactMatch = true, bool? mutuallyExclusive = false, bool? includeNavigationProperties = true)
        {
            if (filterBy?.Any() ?? false)
                return await _firstracApiHelper.PostAsync<Dictionary<string, object>, APIOperationResult<IEnumerable<IAccountType>>>($"api/AccountType/filteredBy?activeOnly={activeOnly}&exactMatch={exactMatch}&mutuallyExclusive={mutuallyExclusive}&includeAllNavigationProperties={includeNavigationProperties}", filterBy);
            else
        ...
        {
            return await _firstracApiHelper.PostAsync<Dictionary<string, object>, APIOperationResult<IEnumerable<IDropdownItem>>>($"api/AccountType/dropdownItems?exactMatch={exactMatch}", filterBy);
        }
