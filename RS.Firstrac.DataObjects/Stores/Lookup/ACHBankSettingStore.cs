...
namespace RS.Firstrac.DataObjects.Stores.Lookup
{
    public class ACHBankSettingStore : StoreBase, IACHBankSettingStore
    {
        ...
        public async Task<IAPIOperationResult<IEnumerable<IACHBankSetting>>> GetAll(bool? activeOnly, Dictionary<string, object>? filterBy = null, bool? exactMatch = true, bool? mutuallyExclusive = false, bool? includeNavigationProperties = true, Dictionary<string,object>? dependencies = null)
        {
            if (filterBy?.Any() ?? false)
            {
                ...
                return await _firstracApiHelper.PostAsync<IGetAllRequest, APIOperationResult<IEnumerable<IACHBankSetting>>>($"api/ACHBankSetting/filteredBy", request);                
            }
        ...
        public override async Task<IAPIOperationResult<IEnumerable<IDropdownItem>>> GetForDropdown(Dictionary<string, object>? filterBy, bool exactMatch = false, Dictionary<string,object>? dependencies = null)
        {
            ...
            return await _firstracApiHelper.PostAsync<IGetForDropdownRequest, APIOperationResult<IEnumerable<IDropdownItem>>>($"api/ACHBankSetting/dropdownItems", request);
        }
