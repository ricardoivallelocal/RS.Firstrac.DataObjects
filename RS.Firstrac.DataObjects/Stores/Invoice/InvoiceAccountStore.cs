...
namespace RS.Firstrac.DataObjects.Stores.Invoice
{
    ...
    public class InvoiceAccountStore : StoreBase, IInvoiceAccountStore
    {
        ...
        public async Task<IAPIOperationResult<IEnumerable<IInvoiceAccount>>> GetAll(bool? activeOnly, Dictionary<string, object>? filterBy = null, bool? exactMatch = true, bool? mutuallyExclusive = false, bool? includeNavigationProperties = true, Dictionary<string,object>? dependencies = null)
        {
            if (filterBy?.Any() ?? false)
            {
                ...
                return await _firstracApiHelper.PostAsync<IGetAllRequest, APIOperationResult<IEnumerable<InvoiceAccount>>>($"api/invoiceAccount/filteredBy", request);               
            }
        ...
        public override async Task<IAPIOperationResult<IEnumerable<IDropdownItem>>> GetForDropdown(Dictionary<string, object>? filterBy, bool exactMatch = false, Dictionary<string, object>? dependencies = null)
        {
            ...
            return await _firstracApiHelper.PostAsync<IGetForDropdownRequest, APIOperationResult<IEnumerable<IDropdownItem>>>($"api/InvoiceAccount/dropdownItems", request);
        }
