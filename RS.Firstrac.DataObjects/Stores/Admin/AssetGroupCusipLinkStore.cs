...
namespace RS.Firstrac.DataObjects.Stores.Admin
{
    ...
    public class AssetGroupCusipLinkStore : StoreBase, IAssetGroupCusipLinkStore
    {
        ...
        public async Task<IAPIOperationResult<IEnumerable<IAssetGroupCusipLink>>> GetAll(bool? activeOnly, Dictionary<string, object>? filterBy = null, bool? exactMatch = true, bool? mutuallyExclusive = false, bool? includeNavigationProperties = true, Dictionary<string,object>? dependencies = null)
        {
            if (filterBy?.Any() ?? false)
            {
                ...
                return await _firstracApiHelper.PostAsync<IGetAllRequest, APIOperationResult<IEnumerable<IAssetGroupCusipLink>>>($"api/AssetGroupCusipLink/filteredBy",request);
            }
        ...
        public override async Task<IAPIOperationResult<IEnumerable<IDropdownItem>>> GetForDropdown(Dictionary<string, object>? filterBy, bool exactMatch = false, Dictionary<string, object>? dependencies = null)
        {
            ...
            return await _firstracApiHelper.PostAsync<IGetForDropdownRequest, APIOperationResult<IEnumerable<IDropdownItem>>>($"api/AssetGroupCusipLink/dropdownItems",request);
        }
