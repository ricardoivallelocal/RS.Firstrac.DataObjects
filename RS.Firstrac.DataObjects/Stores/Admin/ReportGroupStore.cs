...
namespace RS.Firstrac.DataObjects.Stores.Admin
{
    public class ReportGroupStore : StoreBase, IReportGroupStore
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ReportGroupStore"/> class.
        /// </summary>
        ...
        /// </summary>
        /// <returns>IAPIOperationResult&lt;IEnumerable&lt;IReportGroup&gt;&gt;.</returns>
        public async Task<IAPIOperationResult<IEnumerable<IReportGroup>>> GetAll(bool? activeOnly, Dictionary<string, object>? filterBy = null, bool? exactMatch = true, bool? mutuallyExclusive = false, bool? includeNavigationProperties = true, Dictionary<string, object> dependencies = null)
        {
            if (filterBy?.Any() ?? false)
            {
                ...
                return await _firstracApiHelper.PostAsync<IGetAllRequest, APIOperationResult<IEnumerable<IReportGroup>>>($"api/ReportGroup/filteredBy", request);
            }

            else
                return await _firstracApiHelper.GetAsync<APIOperationResult<IEnumerable<IReportGroup>>>($"api/ReportGroup?activeOnly={activeOnly}&includeAllNavigationProperties={includeNavigationProperties}");
        }
        ...
        {
            var request = GetForDropdownRequest.Build(filterBy, exactMatch, dependencies);
            return await _firstracApiHelper.PostAsync<IGetForDropdownRequest, APIOperationResult<IEnumerable<IDropdownItem>>>($"api/ReportGroup/dropdownItemsByWhiteLabelExternalId", request);
        }
        ...
        public async Task<IAPIOperationResult<bool>> Save(IReportGroup model)
        {
            return await _firstracApiHelper.PostAsync<IReportGroup, APIOperationResult<bool>>("api/ReportGroup", model);
        }
        ...
        {
            return await _firstracApiHelper.DeleteAsync<APIOperationResult<bool>>($"api/ReportGroup/{id}?deletedBy={deletedBy}");
        }
        ...
        /// </summary>
        /// <param name="id">The identifier.</param>
        /// <returns>IAPIOperationResult&lt;IReportGroup&gt;.</returns>
        public async Task<IAPIOperationResult<IReportGroup>> Get(int id)
        {
            return await _firstracApiHelper.GetAsync<APIOperationResult<IReportGroup>>("api/ReportGroup/{id}", id);
        }
