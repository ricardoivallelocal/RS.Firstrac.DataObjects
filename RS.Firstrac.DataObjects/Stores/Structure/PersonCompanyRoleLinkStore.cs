...
namespace RS.Firstrac.DataObjects.Stores.Admin
{
    ...
    public class PersonCompanyRoleLinkStore : StoreBase, IPersonCompanyRoleLinkStore
    {
        ...
        public async Task<IAPIOperationResult<IEnumerable<IPersonCompanyRoleLink>>> GetAll(bool? activeOnly, Dictionary<string, object>? filterBy = null, bool? exactMatch = true, bool? mutuallyExclusive = false, bool? includeNavigationProperties = true, Dictionary<string,object>? dependencies = null)
        {
            if (filterBy?.Any() ?? false)
            {
                ...
                return await _firstracApiHelper.PostAsync<IGetAllRequest, APIOperationResult<IEnumerable<IPersonCompanyRoleLink>>>($"api/PersonCompanyRoleLink/filteredBy", request);               
            }
        ...
        public override async Task<IAPIOperationResult<IEnumerable<IDropdownItem>>> GetForDropdown(Dictionary<string, object>? filterBy, bool exactMatch = false, Dictionary<string,object>? dependencies = null)
        {
            ...
            return await _firstracApiHelper.PostAsync<IGetForDropdownRequest, APIOperationResult<IEnumerable<IDropdownItem>>>($"api/PersonCompanyRoleLink/dropdownItems", request);
        }
